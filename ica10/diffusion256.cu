...
void cuda_diffusion(float* u, float *u_new, const unsigned int n){
  ...
  if(idx == 0) u_new[idx] = -u_new[NG+1];
  if(idx == 1) u_new[idx] = -u_new[NG];

  if(idx == n-NG) u_new[idx] = -u_new[idx-1];
  if(idx == n-NG+1) u_new[idx] = -u_new[idx-3];
}
 ...
 *******************************************************************************/
  ...
  float* host_u2 = new float[n];
  //buffer used for u_new
  ...
  outputToFile("data256/host_uInit.dat",host_u,n);
  ...
  for( i = 0 ; i < n_steps; i++){
    ...
    if(outputData && i%outputPeriod == 0){
      sprintf(filename,"data256/host_u%08d.dat",i);
  ...
  outputToFile("data256/host_uFinal.dat",host_u,n);
 ...
 *******************************************************************************/
  ...
  outputToFile("data256/cuda_uInit.dat",cuda_u,n);
  ...
  for( i = 0 ; i < n_steps; i++){
    ...
    if(outputData && i%outputPeriod == 0){
      ...
      sprintf(filename,"data256/cuda_u%08d.dat",i);
    ...
    cuda_diffusion<<<(n+BLOCK_DIM_X-1)/BLOCK_DIM_X,BLOCK_DIM_X>>>(d_u, d_u2, n);
  ...
  outputToFile("data256/cuda_uFinal.dat",cuda_u,n);
