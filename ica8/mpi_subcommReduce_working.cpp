...
int main(int argc, char **argv)
{
    ...
    int rowColor = myVal/rowSize;// PUT CODE HERE
    int colColor = myVal%rowSize;// PUT CODE HERE
    ...
    MPI_Comm_split(MPI_COMM_WORLD, colColor, myVal, &colComm);// PUT CODE HERE

    // Create row comm
    MPI_Comm rowComm;
    MPI_Comm_split(MPI_COMM_WORLD, rowColor, myVal, &rowComm);// PUT CODE HERE

    // Get the rank within the new communicators
    int rowRank, colRank;
    MPI_Comm_rank(rowComm, &rowRank); // PUT CODE HERE
    MPI_Comm_rank(colComm, &colRank);
    // Now perform reduction along rows and columns
    int rowSum, colSum;
    MPI_Allreduce(&rowRank, &rowSum, 1, MPI_INT, MPI_SUM, rowComm);// PUT CODE HERE
    MPI_Allreduce(&colRank, &colSum, 1, MPI_INT, MPI_SUM, colComm);
